...
if __name__ == '__main__':
    ...
    for bkg in bkg_list : 
        ...
        for i in range(0,len(hList)):
            ...
            elif not 'sig' in hname : continue
    ...
    df_dict['Category'] = ["T1tttt Cat.", "TTJets(1l) Cat.","TTJets(2l) Cat.","WJets Cat."]
    df_dict['TTJets(1l)'] = [str(TTl_1)+" +/- "+str(round(err_0,2)),str(TTl_2)+" +/- "+str(round(err_1,2)),str(TTl_3)+" +/- "+str(round(err_4,2)),str(TTl_4)+" +/- "+str(round(err_7,2))]
    df_dict['TTJets(2l)'] = [str(TTll_1)+" +/- "+str(round(err_00,2)),str(TTll_2)+" +/- "+str(round(err_2,2)),str(TTll_3)+" +/- "+str(round(err_5,2)),str(TTll_4)+" +/- "+str(round(err_8,2))]
    df_dict['Others'] = [str(WJ_1)+" +/- "+str(round(err_000,2)),str(WJ_2)+" +/- "+str(round(err_12,2)),str(WJ_3)+" +/- "+str(round(err_13,2)),str(WJ_4)+" +/- "+str(round(err_14,2))]
    df_dict['QCD'] = [str(QCD_1)+" +/- "+str(round(err_15,2)),str(QCD_2)+" +/- "+str(round(err_16,2)),str(QCD_3)+" +/- "+str(round(err_17,2)),str(QCD_4)+" +/- "+str(round(err_18,2))]
    df_dict['Data - QCD_{MC}'] =   ["-- +/- --", str(Data_2-QCD_2)+" +/- "+str(round(sqrt(err_3**2+err_16**2),2)), str(Data_3-QCD_3)+" +/- "+str(round(sqrt(err_6**2+err_17**2),2)), str(Data_4-QCD_4)+" +/- "+str(round(sqrt(err_9**2+err_18**2),2))]

    dftex_dict['Category &'] = ["T1tttt Cat. &", "TTJets(1l) Cat. &","TTJets(2l) Cat. &","WJets Cat. &"]
    dftex_dict['TTJets(1l) &'] = [str(TTl_1)+" $\pm$ "+str(round(err_0,2))+" &",str(TTl_2)+" $\pm$ "+str(round(err_1,2))+" &",str(TTl_3)+" $\pm$ "+str(round(err_4,2))+" &",str(TTl_4)+" $\pm$ "+str(round(err_7,2))+" &"]
    dftex_dict['TTJets(2l) &'] = [str(TTll_1)+" $\pm$ "+str(round(err_00,2))+" &",str(TTll_2)+" $\pm$ "+str(round(err_2,2))+" &",str(TTll_3)+" $\pm$ "+str(round(err_5,2))+" &",str(TTll_4)+" $\pm$ "+str(round(err_8,2))+" &"]
    dftex_dict['Others &'] = [str(WJ_1)+" $\pm$ "+str(round(err_000,2))+" &",str(WJ_2)+" $\pm$ "+str(round(err_12,2))+" &",str(WJ_3)+" $\pm$ "+str(round(err_13,2))+" &",str(WJ_4)+" $\pm$ "+str(round(err_14,2))+" &"]
    dftex_dict['QCD &'] = [str(QCD_1)+" $\pm$ "+str(round(err_15,2))+" &",str(QCD_2)+" $\pm$ "+str(round(err_16,2))+" &",str(QCD_3)+" $\pm$ "+str(round(err_17,2))+" &",str(QCD_4)+" $\pm$ "+str(round(err_18,2))+" &"]
    dftex_dict['Data $- QCD_{MC}$\\\\ \\hline'] =   ["-- $\pm$ -- \\\\ \\hline", str(Data_2-QCD_2)+" $\pm$ "+str(round(sqrt(err_3**2+err_16**2),2))+" \\\\ \\hline", str(Data_3-QCD_3)+" $\pm$ "+str(round(sqrt(err_6**2+err_17**2),2))+" \\\\ \\hline", str(Data_4-QCD_4)+" $\pm$ "+str(round(sqrt(err_9**2+err_18**2),2))+" \\\\"]
    ...
    b = unumpy.umatrix([[Data_2-QCD_2],[Data_3-QCD_3],[Data_4-QCD_4]],[[sqrt(err_3**2+err_16**2)],[sqrt(err_6**2+err_17**2)],[sqrt(err_9**2+err_18**2)]])
    ...
    #print(Y,Yerr)
    alpha, beta , gamma , delta  = round(Y[0],2) , round(Y[1],2),round( Y[2],2),1.0
    alphaerr, betaerr , gammaerr, deltaerr  = round(Yerr[0],2) , round(Yerr[1],2),round( Yerr[2],2),0.0
    ...
    if 1 == 1 :
        ...
        for i in range(0,10):
            ...
            a = unumpy.umatrix([[TTl_2,TTll_2,WJ_2],[TTl_3,TTll_3,WJ_3],[TTl_4,TTll_4,WJ_4]],[[err_1,err_2,err_12],[err_4,err_5,err_13],[err_7,err_8,err_14]])
            b = unumpy.umatrix([[Data_2-QCD_norm*QCD_2],[Data_3-QCD_norm*QCD_3],[Data_4-QCD_norm*QCD_4]],[[sqrt(err_3**2+err_16**2)],[sqrt(err_6**2+err_17**2)],[sqrt(err_9**2+err_18**2)]])
            Y_ = a.I * b
            
            Y = np.squeeze(np.asarray(unumpy.nominal_values(Y_)))
            Yerr = np.squeeze(np.asarray(unumpy.std_devs(Y_)))

            alpha, beta , gamma , delta  = round(Y[0],2) , round(Y[1],2),round( Y[2],2),round(QCD_norm,2)
            alphaerr, betaerr , gammaerr, deltaerr  = round(Yerr[0],2) , round(Yerr[1],2),round( Yerr[2],2),round(0.03,2)

            abgd_dict[i+1] = {}
            abgd_dict[i+1]['alpha'] = alpha;     abgd_dict[i+1]['beta'] = beta; abgd_dict[i+1]['gamma'] = gamma;    abgd_dict[i+1]["delta"] = delta
    ...
    #if not "mucha" in infile : 
    txtalphabetagamma.write("{:<20}{:<12}{:<10}{:<12}{:<10}{:<12}{:<10}{:<12}{:<10}".format(' ',abgd_dict[1]['alpha'], '+/-'+str(alphaerr),abgd_dict[1]['beta'] ,'+/-'+str(betaerr), abgd_dict[1]['gamma'],'+/-'+str(gammaerr),abgd_dict[1]['delta'],'+/-'+str(deltaerr) )+"\n")
    textalphabetagamma.write("{:<20}{:<12}{:<15}{:<12}{:<15}{:<12}{:<15}{:<12}{:<15}".format(' ',abgd_dict[1]['alpha'], '+/-'+str(alphaerr),abgd_dict[1]['beta'] ,'+/-'+str(betaerr), abgd_dict[1]['gamma'],'+/-'+str(gammaerr),abgd_dict[1]['delta'],'+/-'+str(deltaerr) )+"\n")
    #else :
    #    txtalphabetagamma.write("{:<20}{:<12}{:<10}{:<12}{:<10}{:<12}{:<10}{:<12}{:<10}".format(' ',abgd_dict[0]['alpha'], '+/-'+str(alphaerr),abgd_dict[0]['beta'] ,'+/-'+str(betaerr), abgd_dict[0]['gamma'],'+/-'+str(gammaerr),"","" )+"\n")
    #    textalphabetagamma.write("{:<20}{:<12}{:<15}{:<12}{:<15}{:<12}{:<15}{:<12}{:<15}".format(' ',abgd_dict[0]['alpha'], '+/-'+str(alphaerr),abgd_dict[0]['beta'] ,'+/-'+str(betaerr), abgd_dict[0]['gamma'],'+/-'+str(gammaerr),"",'' )+"\n")
    ...
    latexalphabetagamma.write("{:<40}{:<20}{:<10}{:<20}{:<10}{:<20}{:<10}{:<20}{:<10}".format('   & ','$\\alpha$',' & ','$\\beta$' ,' & ', '$\\gamma$',  '&', '','&')+"\\\\ \n")
    latexalphabetagamma.write('\\hline\\hline \n')
    #if not "mucha" in infile : 
    for i in range(0,10):
        latexalphabetagamma.write("{:<40}{:<20}{:<10}{:<20}{:<10}{:<20}{:<10}{:<20}{:<10}".format(' i = '+str(i)+' & ',str(abgd_dict[i]['alpha']), ' $\pm$ '+str(alphaerr)+' & ',str(abgd_dict[i]['beta']) ,' $\pm$ '+str(betaerr)+' & ', str(abgd_dict[i]['gamma']),' $\pm$ '+str(gammaerr)+' &',str(abgd_dict[i]['delta']),'&  \\\\' )+"\n")
        latexalphabetagamma.write('\\hline\\hline\\hline \n')
    #else :
    #    latexalphabetagamma.write("{:<40}{:<20}{:<10}{:<20}{:<10}{:<20}{:<10}{:<20}{:<10}".format('  & ',str(abgd_dict[0]['alpha']), ' $\pm$ '+str(alphaerr)+' & ',str(abgd_dict[0]['beta']) ,' $\pm$ '+str(betaerr)+' & ', str(abgd_dict[0]['gamma']),' $\pm$ '+str(gammaerr)+' &',"",'&  \\\\' )+"\n")
    #    latexalphabetagamma.write('\\hline\\hline\\hline \n') 
